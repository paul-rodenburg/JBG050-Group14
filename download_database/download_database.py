...
if os.path.isfile('../data/crime_data.db'):
    # Check if user has newest version of database (using hashing),
    # if not download the latest database from Google Drive.
    ...
    newest_hash = f.read()
    f.close()
    print("Now hashing your database (in order to get the version), this can take a while ... It will probably take "
...
for database in databases.keys():
    ...
    if os.path.isfile(f'../data/{database}.db'):
        ...
        else:  # If user type something else than y or yes, it will stop..
